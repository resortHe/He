...
// Format 实现Formatter(entry *logers.Entry)([]byte,error)接口
func (t *LogFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	if entry.HasCaller() {
		...
		fmt.Fprintf(b, "%s[%s] \x1b[%dm[%s]\x1b[0m %s %s %s \n", log.Prefix, timestamp, levelColor, entry.Level, filVal, funVal, entry.Message)
	} else {
...
func InitLogger() *logrus.Logger {
	...
	level, err := logrus.ParseLevel(global.Config.Logger.Level) //设置最低Level
	if err != nil {
